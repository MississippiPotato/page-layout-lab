(function () {
  ...
  const CODE = {
    ...
    path: [`const intended = ["chart", "active", "complete", "ranking", "recent"];\nconst actual = [...modules]\n  .sort((a,b) => b.importance - a.importance)\n  .map(d => d.id);`, `const matched = d3.sum(actual.map((id, i) => id === intended[i]));\nconst score = matched / intended.length;\npath.attr("d", line(actual.map(id => center[id])));`],
    ...
    density: [`const layout = makeCards(moduleCount, cardPadding);\nconst centers = layout.map(d => [d.x + d.w/2, d.y + d.h/2]);`, `const crowding = crowdingRisk(layout, cardPadding);\nconst scanDistance = d3.mean(d3.pairs(centers), distance);\nconst firstScreen = d3.mean(layout, d => d.y + d.h <= foldY);`],
    ...
    multiscreen: [`const decisions = {\n  1200: { keep: "all" },\n  768: { move: "chart", collapse: "device" },\n  375: { stack: "core", hide: "device" }\n};`, `const views = [1200, 768, 375].map(width => ({\n  width, cards: layoutByWidth(width, decisions[width])\n}));\nscreenG.data(views, d => d.width).join("g");`]
  };
  ...
  const CODE_META = {
    ...
    density:[B('由模块数量与内边距计算卡片','Compute cards from count and padding'),B('同时计算拥挤、扫描距离与首屏覆盖','Measure crowding, scan distance, and first-screen coverage together'),B('增加模块或减少内边距，同时比较三项指标，避免只优化单一数字。','Add modules or reduce padding and compare all three metrics instead of optimizing one number.')],
    ...
    multiscreen:[B('为每个断点记录布局决策','Record layout decisions for every breakpoint'),B('用同一内容模型生成多屏布局','Generate multi-screen layouts from one content model'),B('对照三屏说明，验证保留、移动、折叠、堆叠与隐藏是否和画面一致。','Compare the three views with their rationale and verify every keep, move, collapse, stack, and hide decision.')]
  };
  ...
  function highlight(code) {
    const token=/\/\/[^\n]*|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|\b(?:const|let|if|else|return|new|for|of|function|true|false)\b|\b\d+(?:\.\d+)?\b/g;
    let out='',last=0;
    code.replace(token,(match,offset)=>{
      out+=esc(code.slice(last,offset));
      const cls=match.startsWith('//')?'cmt':/^['"]/.test(match)?'str':/^\d/.test(match)?'num':'fn';
      out+=`<span class="${cls}">${esc(match)}</span>`;last=offset+match.length;
      return match;
    });
  ...
  function hierarchyControls(data,showPath,compareOrder=false){
    const metrics=compareOrder?`<div class="metric-list order-evidence"><div class="metric-row"><span>${lang==='zh'?'期望顺序':'Intended order'}</span><b id="expectedOrder">—</b></div><div class="metric-row"><span>${lang==='zh'?'实际显著性':'Actual salience'}</span><b id="actualOrder">—</b></div><div class="metric-row"><span>${lang==='zh'?'顺序匹配':'Order match'}</span><b id="orderMatch">—</b><i><b id="orderBar"></b></i></div></div>`:'';
    return data.slice(0,5).map(d=>range(`imp-${d.id}`,nameOf(d.id),1,5,d.importance)).join('')+`<label class="toggle-row"><input id="showRead" type="checkbox" ${showPath?'checked':''}> ${lang==='zh'?'显示启发式阅读路径':'Show heuristic reading path'}</label>${metrics}`;
  }
  function demoHierarchy(section,forcePath=false) {
    const data=baseModules().slice(0,5), intended=['chart','active','complete','ranking','recent'];
    const b=demoBase(section,hierarchyControls(data,forcePath,forcePath)),svg=b.svg;
    defs(svg);svg.append('rect').attr('class','svg-frame').attr('x',15).attr('y',15).attr('width',690).attr('height',400).attr('rx',10);
    const update=()=>{
      const layout=hierarchyLayout(data),actual=[...data].sort((a,z)=>z.importance-a.importance||intended.indexOf(a.id)-intended.indexOf(z.id)).map(d=>d.id);
      const cards=svg.selectAll('g.hcard').data(layout,d=>d.id).join(enter=>{const g=enter.append('g').attr('class','hcard');g.each(function(d){drawCard(d3.select(this),d)});return g;});
      cards.transition().duration(450).attr('transform',d=>`translate(${d.x},${d.y})`);
      cards.select('rect').transition().duration(450).attr('width',d=>d.w).attr('height',d=>d.h).attr('class',d=>d.importance===5?'svg-card main':'svg-card');
      const show=b.controls.querySelector('#showRead').checked,byId=Object.fromEntries(layout.map(d=>[d.id,d]));
      const pts=actual.map(id=>[byId[id].x+byId[id].w/2,byId[id].y+byId[id].h/2]);
      svg.selectAll('path.reading-path').data(show?[pts]:[]).join('path').attr('class','reading-path').attr('d',d3.line().curve(d3.curveCatmullRom.alpha(.35)));
      if(forcePath){
        const matched=d3.sum(actual.map((id,i)=>id===intended[i]?1:0)),score=matched/intended.length*100,short=ids=>ids.map(id=>nameOf(id)).join(' → ');
        b.controls.querySelector('#expectedOrder').textContent=short(intended);
        b.controls.querySelector('#actualOrder').textContent=short(actual);
        b.controls.querySelector('#orderMatch').textContent=`${Math.round(score)}%`;
        b.controls.querySelector('#orderBar').style.width=`${score}%`;
        b.controls.querySelector('#orderBar').style.background=score===100?'var(--success)':'var(--coral)';
        setStatus(b,score===100?'实际显著性顺序与任务期望一致。调高次要模块，观察路径冲突。':`有 ${intended.length-matched} 个位置与期望不一致；应调整面积、位置或对比。`,score===100?'Actual salience matches the intended task order. Raise a secondary module to create a conflict.':`${intended.length-matched} positions conflict with the intended order; adjust area, position, or contrast.`);
      }else setStatus(b,show?'路径按视觉重量排序；它解释设计意图，不是眼动数据。':'提高模块重要性，观察它获得的面积与位置。',show?'The path follows visual weight; it explains intent, not eye-tracking data.':'Raise a module’s importance and watch it gain area and position.');
    };
    data.forEach(d=>{const el=b.controls.querySelector(`#imp-${d.id}`);el.addEventListener('input',()=>{d.importance=+el.value;b.controls.querySelector(`#imp-${d.id}Out`).textContent=el.value;update()})});
    b.controls.querySelector('#showRead').addEventListener('change',update);update();
  }
  ...
  function demoDensity(section){
    let count=7,pad=16;
    const metric=(label,id)=>`<div class="metric-row"><span>${label}</span><b id="${id}">—</b><i><b id="${id}Bar"></b></i></div>`;
    const b=demoBase(section,range('densityCount',lang==='zh'?'模块数量':'Module count',4,12,count)+range('densityPad',lang==='zh'?'卡片内边距':'Card padding',6,30,pad)+`<div class="metric-list">${metric(lang==='zh'?'拥挤风险':'Crowding risk','crowdRisk')}${metric(lang==='zh'?'扫描距离':'Scan distance','scanDistance')}${metric(lang==='zh'?'首屏覆盖率':'First-screen coverage','foldCoverage')}</div>`),svg=b.svg;
    const update=()=>{
      svg.selectAll('*').remove();
      svg.append('rect').attr('class','svg-frame').attr('x',22).attr('y',18).attr('width',676).attr('height',394).attr('rx',10);
      const cols=count>8?4:count>5?3:2,gap=Math.max(5,30-pad*.75),cw=(620-gap*(cols-1))/cols;
      const data=d3.range(count).map(i=>({id:i,x:50+(i%cols)*(cw+gap),y:52+Math.floor(i/cols)*(88+gap),w:cw,h:88}));
      const g=svg.selectAll('g.dcard').data(data,d=>d.id).join('g').attr('transform',d=>`translate(${d.x},${d.y})`);
      g.append('rect').attr('class','svg-card').attr('rx',6).attr('width',d=>d.w).attr('height',d=>d.h);
      g.append('text').attr('class','svg-meta').attr('x',pad).attr('y',pad+10).text((d,i)=>`${lang==='zh'?'模块':'MODULE'} ${i+1}`);
      g.append('line').attr('x1',pad).attr('x2',d=>Math.max(pad+15,d.w-pad)).attr('y1',pad+30).attr('y2',pad+30).attr('stroke','#c7cfdd').attr('stroke-width',5).attr('stroke-linecap','round');
      const centers=data.map(d=>[d.x+d.w/2,d.y+d.h/2]),distances=d3.pairs(centers).map(([a,z])=>Math.hypot(z[0]-a[0],z[1]-a[1]));
      const crowd=Math.min(100,Math.max(0,(count-5)*10+(16-pad)*2)),scan=Math.round(d3.mean(distances)||0),coverage=data.filter(d=>d.y+d.h<=394).length/count*100;
      const metrics=[['crowdRisk',crowd,100-crowd],['scanDistance',scan,Math.max(0,100-scan/5)],['foldCoverage',coverage,coverage]];
      metrics.forEach(([id,value,bar])=>{b.controls.querySelector(`#${id}`).textContent=id==='scanDistance'?`${value}px`:`${Math.round(value)}%`;const fill=b.controls.querySelector(`#${id}Bar`);fill.style.width=`${bar}%`;fill.style.background=bar>65?'var(--success)':bar>40?'var(--gold)':'var(--coral)'});
      svg.append('line').attr('class','fold-line').attr('x1',34).attr('x2',686).attr('y1',394).attr('y2',394);
      svg.append('text').attr('class','fold-label').attr('x',680).attr('y',386).attr('text-anchor','end').text(lang==='zh'?'首屏边界':'FIRST-SCREEN FOLD');
      setStatus(b,crowd<40&&coverage===100?`拥挤 ${Math.round(crowd)}%、扫描 ${scan}px、首屏覆盖 ${Math.round(coverage)}%；三项共同处于可用区间。`:`拥挤 ${Math.round(crowd)}%、扫描 ${scan}px、首屏覆盖 ${Math.round(coverage)}%；应合并模块或渐进披露。`,crowd<40&&coverage===100?`Crowding ${Math.round(crowd)}%, scan distance ${scan}px, first-screen coverage ${Math.round(coverage)}%; all three remain usable.`:`Crowding ${Math.round(crowd)}%, scan distance ${scan}px, first-screen coverage ${Math.round(coverage)}%; group modules or disclose progressively.`);
    };
    [['densityCount','count'],['densityPad','pad']].forEach(([id,key])=>{const el=b.controls.querySelector('#'+id);el.oninput=()=>{key==='count'?count=+el.value:pad=+el.value;b.controls.querySelector('#'+id+'Out').textContent=el.value;update()}});update();
  }
  ...
  function layoutForWidth(w,priority=false){
    const ids=priority?['chart','active','complete','ranking','recent']:['active','complete','ranking','chart','recent'],values={chart:'8.4K',active:'8,392',complete:'72%',ranking:'#1',recent:'24'},pad=24,gap=14;
    if(w<520){let y=48;return ids.map(id=>{const h=id==='chart'?108:54,out={id,x:pad,y,w:w-pad*2,h,value:values[id],main:id==='chart'};y+=h+gap;return out})}
    if(w<850){const cw=(w-pad*2-gap)/2,rows=[];for(let i=0;i<ids.length;i+=2)rows.push(ids.slice(i,i+2));let y=52;return rows.flatMap(row=>{const h=d3.max(row,id=>id==='chart'?150:74),out=row.map((id,i)=>({id,x:pad+i*(cw+gap),y,w:cw,h:id==='chart'?150:74,value:values[id],main:id==='chart'}));y+=h+gap;return out})}
    const cw=(w-pad*2-gap*3)/4;return [{id:'active',x:pad,y:52,w:cw,h:66,value:'8,392'},{id:'complete',x:pad+cw+gap,y:52,w:cw,h:66,value:'72%'},{id:'ranking',x:pad+2*(cw+gap),y:52,w:cw,h:66,value:'#1'},{id:'recent',x:pad+3*(cw+gap),y:52,w:cw,h:66,value:'24'},{id:'chart',x:pad,y:136,w:cw*3+gap*2,h:190,value:'8.4K',main:true},{id:'device',x:pad+3*(cw+gap),y:136,w:cw,h:190,value:'62%'}]
  }
  function demoResponsiveCore(section,priority=false){
    let width=priority?430:980;
    const orderMetrics=priority?`<div class="metric-row"><span>${lang==='zh'?'视觉顺序':'Visual order'}</span><b id="visualOrder">—</b></div><div class="metric-row"><span>${lang==='zh'?'DOM / 焦点顺序':'DOM / focus order'}</span><b id="domOrder">—</b></div><div class="metric-row"><span>${lang==='zh'?'顺序一致性':'Order consistency'}</span><b id="orderConsistency">—</b><i><b id="consistencyBar"></b></i></div>`:'';
    const b=demoBase(section,range('viewportWidth',lang==='zh'?'视口宽度':'Viewport width',320,1200,width)+`<div class="metric-list"><div class="metric-row"><span>${lang==='zh'?'布局模式':'Layout mode'}</span><b id="modeOut">—</b><i><b style="width:100%"></b></i></div>${orderMetrics}</div>`),svg=b.svg;
    const update=()=>{
      svg.selectAll('*').remove();
      const mode=width<520?'MOBILE · 1 COL':width<850?'TABLET · 2 COL':'DESKTOP · 4 COL',frameW=640,frameX=40;
      svg.append('rect').attr('x',frameX).attr('y',20).attr('width',frameW).attr('height',392).attr('rx',12).attr('fill','#e8ebf3').attr('stroke','#cfd6e5');
      const scale=(frameW-16)/width,viewport=svg.append('g').attr('transform',`translate(${frameX+8},30) scale(${scale})`);
      viewport.append('rect').attr('width',width).attr('height',370/scale).attr('fill','#fff').attr('rx',8/scale);
      const data=layoutForWidth(width,priority),g=viewport.selectAll('g.rcard').data(data,d=>d.id).join('g').attr('transform',d=>`translate(${d.x},${d.y})`).attr('tabindex',priority?0:null).attr('aria-label',d=>priority?`${nameOf(d.id)} · ${lang==='zh'?'焦点顺序':'focus order'}`:null);
      g.append('rect').attr('class',d=>d.id==='chart'?'svg-card main':'svg-card').attr('rx',6/scale).attr('width',d=>d.w).attr('height',d=>d.h).attr('stroke-width',1.4/scale);
      g.append('text').attr('class','svg-label').attr('x',10).attr('y',20).attr('font-size',Math.max(12,14/scale)).text(d=>nameOf(d.id));
      b.controls.querySelector('#modeOut').textContent=mode;
      if(priority){
        const visual=[...data].sort((a,z)=>a.y-z.y||a.x-z.x).map(d=>d.id),dom=data.map(d=>d.id),same=visual.every((id,i)=>id===dom[i]),short=ids=>ids.map(id=>nameOf(id)).join(' → ');
        b.controls.querySelector('#visualOrder').textContent=short(visual);
        b.controls.querySelector('#domOrder').textContent=short(dom);
        b.controls.querySelector('#orderConsistency').textContent=same?(lang==='zh'?'一致':'MATCH'):(lang==='zh'?'冲突':'CONFLICT');
        b.controls.querySelector('#consistencyBar').style.width=same?'100%':'28%';
        b.controls.querySelector('#consistencyBar').style.background=same?'var(--success)':'var(--coral)';
        setStatus(b,same?'视觉顺序与 DOM / 焦点顺序一致，键盘用户会按同一路径访问。':'当前宽度出现顺序冲突；不能只用 CSS order 重排，应同步内容与焦点顺序。',same?'Visual and DOM/focus order match, so keyboard users follow the same path.':'This width exposes an order conflict. Do not reflow with CSS order alone; synchronize content and focus order.');
      }else setStatus(b,'卡片不是整体缩小，而是在阈值处改变列数与顺序。','Cards do not scale down as a whole; columns and order change at thresholds.');
    };
    const el=b.controls.querySelector('#viewportWidth');el.oninput=()=>{width=+el.value;b.controls.querySelector('#viewportWidthOut').textContent=`${width}px`;update()};b.controls.querySelector('#viewportWidthOut').textContent=`${width}px`;update();
  }
  ...
  function demoMultiscreen(section){
    let zoom=100;
    const decisions=lang==='zh'?[['桌面 1200','保留：6 个模块；四列 + 主辅栏'],['平板 768','移动：主图提前；折叠：设备分布'],['手机 375','堆叠：5 个核心模块；隐藏：设备分布']]:[['Desktop 1200','Keep: 6 modules; four columns + main/side'],['Tablet 768','Move: chart first; collapse: device mix'],['Mobile 375','Stack: 5 core modules; hide: device mix']];
    const evidence=`<div class="breakpoint-decisions">${decisions.map(([size,rule])=>`<div><b>${size}</b><span>${rule}</span></div>`).join('')}</div>`;
    const b=demoBase(section,range('multiZoom',lang==='zh'?'文字缩放':'Text zoom',100,200,zoom,10)+`<p class="control-note">${lang==='zh'?'一套内容模型，同时生成桌面、平板和手机视图。':'One content model generates desktop, tablet, and mobile views.'}</p>${evidence}`),svg=b.svg;
    const screens=[{name:'DESKTOP',w:1180,x:24,drawW:315,action:lang==='zh'?'保留':'KEEP'},{name:'TABLET',w:768,x:354,drawW:190,action:lang==='zh'?'移动 + 折叠':'MOVE + COLLAPSE'},{name:'MOBILE',w:375,x:563,drawW:135,action:lang==='zh'?'堆叠 + 隐藏':'STACK + HIDE'}];
    const update=()=>{
      svg.selectAll('*').remove();
      screens.forEach(s=>{
        const scale=s.drawW/s.w,frameH=350;
        svg.append('rect').attr('x',s.x).attr('y',52).attr('width',s.drawW).attr('height',frameH).attr('rx',8).attr('fill','#fff').attr('stroke','#cfd6e5');
        svg.append('text').attr('class','svg-meta screen-title').attr('x',s.x).attr('y',31).text(`${s.name} · ${s.w}px`);
        svg.append('text').attr('class','screen-action').attr('x',s.x).attr('y',45).text(s.action);
        const g=svg.append('g').attr('transform',`translate(${s.x},62) scale(${scale})`);
        let cards=layoutForWidth(s.w,true);
        if(s.w===768)cards=[...cards,{id:'device',x:24,y:390,w:720,h:46,value:'62%',collapsed:true}];
        cards=cards.map(d=>({...d,y:d.y*(zoom/100),h:d.h*(zoom/100)}));
        const cg=g.selectAll('g').data(cards,d=>d.id).join('g').attr('transform',d=>`translate(${d.x},${d.y})`);
        cg.append('rect').attr('class',d=>d.collapsed?'svg-card collapsed':d.main?'svg-card main':'svg-card').attr('width',d=>d.w).attr('height',d=>d.h).attr('rx',6/scale);
        cg.append('text').attr('class','svg-label').attr('x',10).attr('y',20).attr('font-size',Math.max(12,14/scale)).text(d=>d.collapsed?`${nameOf(d.id)} · ${lang==='zh'?'已折叠':'COLLAPSED'}`:nameOf(d.id));
      });
      setStatus(b,zoom>160?'文字接近 200%：平板折叠项与固定高度成为重点复核对象。':'断点说明已逐一记录保留、移动、折叠、堆叠和隐藏决策。',zoom>160?'Near 200% text: recheck the tablet disclosure and every fixed height.':'The breakpoint rationale now records keep, move, collapse, stack, and hide decisions explicitly.');
    };
    const el=b.controls.querySelector('#multiZoom');el.oninput=()=>{zoom=+el.value;b.controls.querySelector('#multiZoomOut').textContent=`${zoom}%`;update()};b.controls.querySelector('#multiZoomOut').textContent=`${zoom}%`;update();
  }
